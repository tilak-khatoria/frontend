...
export default function DeclinedComplaints() {
  ...
  const fetchComplaints = async () => {
    try {
      ...
      // Filter for declined/rejected statuses
      const declinedComplaints = Array.isArray(data) 
        ? data.filter(c => ['DECLINED', 'REJECTED'].includes(c.status))
        : [];
      setComplaints(declinedComplaints);
    } catch (error) {
  ...
  const getStatusBadge = (status) => {
    ...
    const style = statusStyles[status] || statusStyles['DECLINED'];
    return (
      <span style={{
        padding: '4px 12px',
        borderRadius: '12px',
      ...
      <main style={styles.main}>
        <div style={styles.content}>
          ...
          <div style={styles.header}>
            <div>
              <h1 style={styles.title}>❌ Declined/Rejected Complaints</h1>
              <p style={styles.subtitle}>
                Complaints that were declined or rejected ({complaints.length} total)
              </p>
            </div>
            <Link href="/dashboard">
          ...
          {complaints.length === 0 ? (
            <div className="card" style={styles.emptyState}>
              ...
              <h3 style={styles.emptyTitle}>No declined complaints</h3>
              <p style={styles.emptyText}>No complaints have been declined or rejected.</p>
            </div>
          ) : (
