...
export default function RejectedComplaints() {
  ...
  const fetchComplaints = async () => {
    try {
      ...
      // Filter for rejected/declined statuses
      const rejectedStatuses = ['DECLINED', 'REJECTED'];
      allComplaints = allComplaints.filter(c => rejectedStatuses.includes(c.status));
  ...
  if (loading || loadingData) {
    return (
      <div style={styles.loadingContainer}>
        ...
        <p style={styles.loadingText}>Loading rejected complaints...</p>
      </div>
    );
  }
  ...
  return (
    <>
      <Head>
        <title>Rejected Complaints - Admin - CivicSaathi</title>
      </Head>
        ...
        <main style={styles.main}>
          <div style={styles.content}>
            <div style={styles.header}>
              <div>
                <h1 style={styles.title}>❌ Rejected Complaints</h1>
                <p style={styles.subtitle}>
                  {complaints.length} complaint{complaints.length !== 1 ? 's' : ''} declined or rejected
                </p>
              </div>
            ...
            {complaints.length === 0 ? (
              <div style={styles.emptyState}>
                ...
                <h3 style={styles.emptyTitle}>No Rejected Complaints</h3>
                <p style={styles.emptyText}>No complaints have been rejected or declined.</p>
              </div>
            ) : (
                      ...
                      <div style={styles.cardDetails}>
                        ...
                        {complaint.rejection_reason && (
                          <div style={styles.detailItem}>
                            <span style={styles.detailIcon}>💬</span>
                            <span style={styles.detailText}>
                              {complaint.rejection_reason.length > 50
                                ? complaint.rejection_reason.substring(0, 50) + '...'
                                : complaint.rejection_reason}
                            </span>
                          </div>
                        )}
                        ...
                        <div style={styles.detailItem}>
                          <span style={styles.detailIcon}>📅</span>
                          <span style={styles.detailText}>
                            Rejected: {new Date(complaint.updated_at || complaint.created_at).toLocaleDateString()}
                          </span>
                        </div>
                      </div>
